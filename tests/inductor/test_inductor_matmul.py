...
class TestMatmulOps:
    ...
    def test_mm_fp16_overflow(self, execution_mode, scale, expect_inf):
        ...
        if not expect_inf:
            pytest.xfail(
                reason="fp16 mm scale produces wrong results on Spyre: "
                "spyre <-> cpu mismatch for scale_1e2 and scale_1e4 (Issue #1793)"
            )
        # TODO: ISSUE https://github.com/torch-spyre/torch-spyre/issues/1793
        if expect_inf:
            pytest.xfail(
                reason="fp16 mm overflow: Spyre returns nan instead of inf "
                "for scale=1e5 inputs (Issue #1793)"
            )
